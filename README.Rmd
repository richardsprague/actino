...
library(tidyverse)
# the "sprague.map" variable below contains data that is read by these lines.
DATA_DIR <- Sys.getenv("UBIOME_DATADIR")
load(file.path(DATA_DIR,"ubiome_people","ub_data-Richard_Sprague","derived/spragueAllRanks.RData"))
...
```{r exampleRSetup1, eval = FALSE, echo=TRUE}
...
```{r exampleRSetup, eval = FALSE, echo=TRUE}
library(jsonlite)
...
```{r analyzeRMapFile, echo = FALSE}

mapfile <- data.frame
sprague.map %>% select(Username,Date,SSR,Site,Notes,Label) %>% filter(SSR %in% c(15372,15759,60454,27127)) %>% knitr::kable(caption="A Typical Mapfile (Excel)")

```

Load a standard uBiome-downloaded CSV file using this command: 

```{r experimentphyloseq, eval = FALSE}
...
```{r heatmapDemo, message=FALSE, echo=TRUE}
